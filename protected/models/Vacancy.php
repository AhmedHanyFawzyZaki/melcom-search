...
class Vacancy extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('title, intro', 'required'),
			array('title, intro', 'length', 'max'=>255),
			array('details', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, title, intro, details', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('Lower(intro)',strtolower($this->intro),true);
